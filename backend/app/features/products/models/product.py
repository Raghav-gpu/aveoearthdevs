...
class Product(BaseUUID, BaseTimeStamp, Base):
    ...
    status = Column(Enum(ProductStatusEnum, native_enum=False), default=ProductStatusEnum.DRAFT, index=True)
    approval_status = Column(Enum(ProductApprovalEnum, native_enum=False), default=ProductApprovalEnum.PENDING, index=True)
    approval_notes = Column(Text)
    approved_at = Column(DateTime)
    approved_by = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"))
    visibility = Column(Enum(ProductVisibilityEnum, native_enum=False), default=ProductVisibilityEnum.VISIBLE, index=True)
